"""
Belief in Inevitable Conflict — Item-Level Distribution Shapes
================================================================
Two-row × 4-column figure: top row = Russian, bottom row = Estonian, each
column = one of the four BiC items. Same visual grammar as the SI density
charts — KDE + integer-centered overlapping bars.

Direction handling (so every panel reads "higher x = stronger belief in
conflict"):

  - Q63_1 / K6X1_1 ("Ethnic conflicts are inevitable")
        raw 1 = strongly agree = HIGH conflict belief.
        INVERT via 5 - raw so display 4 = strongly agree (high conflict).
  - Q63_2 / K6X1_2 ("Differences divide Estonian society")
        Same as Q63_1 — INVERT.
  - Q63_3 / K6X1_3 ("Different ethnic groups can cooperate")
        raw 1 = strongly agree (groups CAN cooperate) = LOW conflict belief.
        Use RAW directly so display 4 = strongly disagree (cannot cooperate
        = high conflict belief).
  - Q63_4 / K6X1_4 ("Immigration enriches cultural life")
        Same as Q63_3 — use RAW directly.

Output: viz/fig_bic_item_density.jpg  (300 DPI, both groups in one figure)
"""
...
ITEMS = [
    ("Conflicts inevitable",        "Q63_1", "K6X1_1", True),
    ("Differences divide society",  "Q63_2", "K6X1_2", True),
    ("Groups can cooperate",        "Q63_3", "K6X1_3", False),
    ("Immigration enriches life",   "Q63_4", "K6X1_4", False),
]
...
# ---------- Build group-by-item series matrix ------------------------------
data = {}
for grp, ethval, df_year in [
    ("rus", 1, df23), ("rus20", 1, df20), ("est", 0, df23), ("est20", 0, df20),
]:
    pass
...
# ---------- Diagnostics -----------------------------------------------------
print("="*92)
print("BELIEF IN INEVITABLE CONFLICT — item-level diagnostics")
print("(All items oriented so higher = stronger belief in conflict)")
print("="*92)
for label, c23, c20, do_inv in ITEMS:
...
fig, axes = plt.subplots(2, 4, figsize=(13.5, 7.5), dpi=300, sharey="row")
...
for col, (label, c23, c20, do_inv) in enumerate(ITEMS):
    ...
    ax.set_xticks([1, 2, 3, 4])
    ax.set_xticklabels(["1", "2", "3", "4"])
    ax.set_xlabel("Score (1 = weak, 4 = strong belief in conflict)",
...
# Title and footer
fig.text(0.03, 0.965,
         "Belief in Inevitable Conflict — Item-Level Distribution Shapes 2020 → 2023",
         fontsize=13, fontweight="bold", ha="left")
fig.text(0.03, 0.940,
         "Top row: Russian respondents (orange).  Bottom row: Estonian respondents (blue).  "
         "All items oriented so higher x = stronger belief that ethnic conflict is unavoidable.",
         fontsize=9, color="#444")

fig.text(0.03, 0.030,
         "(inv.) panels show 5 - raw so 'strongly agree' (raw 1) appears at x = 4. Q63_3 / Q63_4 panels show raw values directly: high score on these means strong DISAGREEMENT",
         fontsize=7, color="#555")
fig.text(0.03, 0.015,
         "with the positively-worded statements ('groups can cooperate' / 'immigration enriches life'), which on the conflict-belief direction is the high end. Bars are histogram density, curves are Gaussian KDE.",
         fontsize=7, color="#555")

plt.subplots_adjust(left=0.05, right=0.985, top=0.88, bottom=0.10,
                    wspace=0.20, hspace=0.55)

out = ROOT / "viz" / "fig_bic_item_density.jpg"
