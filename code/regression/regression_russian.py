...
# Filter to Russian respondents only
df = df[df["ethnicity_binary"] == 1].copy()
print(f"Russian respondents: {len(df)}")
...
# Q71_1, Q71_3 (language ability): 9 = Don't know → NaN
df["Q71_1"] = df["Q71_1"].replace(9, np.nan)
...
predictors = [
    ...
    "born_elsewhere",
    "Q71_1",              # Estonian language ability (1=native to 6=none)
...
# Drop zero-variance predictors (no variation among Russian respondents)
...
print("\n" + "="*70)
print("OLS REGRESSION: Predictors of Belonging Among Russian Respondents")
