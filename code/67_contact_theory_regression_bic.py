"""
Contact-Theory Regression Check — Belief in Inevitable Conflict
=================================================================
Mirror of scripts 63 and 65, with BiC as the DV. Same four cells (Russian /
Estonian × 2020 / 2023), same single-equation linear model.

Tests whether within-group contact with the primary out-group predicts
weaker belief that ethnic conflict is inevitable.

Direction handling:
  - Contact (Q51 / Q52, K4X1 / K4X2): inverted via (6 − raw) → higher = more contact
  - BiC composite (Option B recoding, per §31e): Q63_1 and Q63_2 reverse-coded
    via (5 − raw), Q63_3 and Q63_4 kept raw. Higher composite = stronger belief
    in conflict. Same convention as code/_effect_sizes.tsv canonical values.
  - Contact theory predicts NEGATIVE slope: more contact → less belief in conflict
    (same sign convention as the SD: Primary regression).

Symmetric construct: BiC asks about ethnic conflict in society at large, so the
contact-theory prediction is symmetric across groups — no asymmetry caveat
like the MIS regression.

Outputs:
  Console:  coefficient, SE, 95% CI, t, p, R², N for each regression
  reports/Contact_Theory_Regression_BiC.docx — formatted Word doc
  viz/fig_contact_theory_regression_bic.jpg — 2×2 scatter+fit panels
"""
...
def to_num_clean(s, dk=9):
    s = pd.to_numeric(s, errors="coerce")
    return s.where(s != dk).astype(float)


def bic_composite(df, items, reverse_items, scale_max=4):
    """Build BiC composite using Option B recoding.

    Items in `reverse_items` are reverse-coded via (scale_max + 1) − raw
    before averaging. Result direction: higher = more belief in conflict.
    Pairwise deletion (skipna mean across items per respondent).
    """
...
def contact_composite(df, items):
    """Inverted contact composite: higher = more contact."""
    sub = df[items].apply(pd.to_numeric, errors="coerce").where(lambda x: x != 9)
...
# ---------- Run regressions -------------------------------------------------
print("=" * 92)
print("CONTACT-THEORY REGRESSION CHECK — Belief in Inevitable Conflict")
print("Model: BiC (Option B: higher = more conflict belief) ~ Contact (higher = more contact)")
print("Contact theory predicts NEGATIVE slope: more contact → less conflict belief")
print("=" * 92)
...
H("Contact-Theory Regression Check — Belief in Inevitable Conflict",
  size=14, color=RGBColor(0x1f, 0x29, 0x37))

body(
    "Targeted bivariate regression of Belief in Inevitable Conflict on frequency of contact "
    "with the primary out-group, run separately for each group × year cell. Companion to the "
    "SD: Primary Out-group regression (Contact_Theory_Regression.docx) and the Minority "
    "Inclusion Support regression (Contact_Theory_Regression_MIS.docx). Tests whether more "
    "contact predicts weaker belief that ethnic conflict is inevitable.",
    italic=True
)
doc.add_paragraph()
...
doc.add_paragraph()
body(
    "Note. Each row reports an OLS regression of the Belief in Inevitable Conflict composite "
    "(Option B recoding per SESSION_LOG §31e: Q63_1 and Q63_2 reverse-coded via 5 − raw; Q63_3 "
    "and Q63_4 kept raw; higher composite = stronger belief in inevitable conflict) on the "
    "Contact composite (inverted via 6 − raw, so higher = more frequent contact). For Russian "
    "respondents, contact is with Estonian-speakers (Q51 / K4X1); for Estonian respondents, "
    "with Russian-speakers (Q52 / K4X2). Contact theory predicts a NEGATIVE slope (more "
    "contact → less belief in conflict). Listwise deletion. Cross-sectional design; "
    "associations are not causal. Significance: *** p < .001, ** p < .01, * p < .05, "
    "ns = not significant.",
    italic=True, size=8
)

out_doc = ROOT / "reports" / "Contact_Theory_Regression_BiC.docx"
doc.save(out_doc)
...
for r, df_pair, color, ax in panel_config:
    ...
    ax.scatter(df_pair["Contact"] + jx, df_pair["BiC"] + jy,
    ...
    ax.set_xlim(0.7, 5.3)
    ax.set_ylim(0.7, 4.3)            # BiC is 1-4 scale
...
axes[1, 1].set_xlabel("Contact composite (inverted: higher = more frequent contact)",
                       fontsize=9, color="#444")
axes[0, 0].set_ylabel("Belief in Inevitable Conflict (higher = more conflict belief)",
                       fontsize=9, color="#444")
axes[1, 0].set_ylabel("Belief in Inevitable Conflict (higher = more conflict belief)",
                       fontsize=9, color="#444")

fig.text(0.04, 0.965,
         "Contact-Theory Regression Check — Belief in Inevitable Conflict ~ Contact",
...
fig.text(0.04, 0.940,
         ...
         fontsize=9, color="#444")
fig.text(0.04, 0.922,
         "Contact theory predicts a NEGATIVE slope: more contact → less belief in inevitable conflict.",
         fontsize=9, color="#444")
...
out_jpg = ROOT / "viz" / "fig_contact_theory_regression_bic.jpg"
