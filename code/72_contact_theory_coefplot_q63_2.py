"""
Contact-Theory Regression Coefficient Plot — Q63_2 "Differences divide society"
==================================================================================
Coefficient (forest) plot variant of script 71. Same four item-level regressions;
slopes aligned on a common axis with 95% CI brackets.

Direction reminder:
  - Q63_2 inverted via (5 − raw); higher = more agreement that differences divide society.
  - Contact inverted via (6 − raw); higher = more contact.
  - Contact theory predicts NEGATIVE slope.

Output: viz/fig_contact_theory_coefplot_q63_2.jpg  (300 DPI)
"""
...
for label, df_pair, group in CELLS:
    ...
    model = sm.OLS(df_pair["Q63_2"], X).fit()
...
ax.set_xticks(np.arange(-0.40, 0.55, 0.05))
ax.set_xlabel("Slope b₁ — change in Q63_2 (inverted) per +1 unit of contact frequency",
...
for x in np.arange(-0.40, 0.55, 0.05):
    ax.axvline(x, color="#eee", linewidth=0.5, zorder=0)

# Title
fig.text(0.04, 0.965,
         "Contact-Theory Regression — Slopes with 95% Confidence Intervals (Q63_2 \"Differences divide society\")",
         fontsize=13, fontweight="bold", ha="left")
fig.text(0.04, 0.940,
         "Bivariate OLS regression of Q63_2 (inverted: higher = more agreement that differences divide) on Contact (inverted: higher = more contact), within each group × year cell.",
         fontsize=8.5, color="#444", ha="left")
fig.text(0.04, 0.918,
         "Negative slope = contact-theory direction (more contact predicts less agreement that intercultural differences divide society).",
         fontsize=8.5, color="#444", ha="left")
...
zero = mlines.Line2D([], [], color="#666", linewidth=1.0,
                     label="0 = no contact–item association")
...
# Footer
fig.text(0.04, 0.025,
         "Item-level analysis (single survey item, no composite). Dashed within-group lines connect 2020 → 2023. Cross-sectional associations; no causal interpretation. EIM is a repeated cross-section, not a panel.",
...
out_path = ROOT / "viz" / "fig_contact_theory_coefplot_q63_2.jpg"
