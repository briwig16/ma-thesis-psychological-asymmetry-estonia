"""
Contact-Theory Regression Check — Superordinate Identity
==========================================================
Mirror of scripts 63 / 65 / 67, with Superordinate Identity (SI) as the DV.
Same four cells (Russian / Estonian × 2020 / 2023), same single-equation
linear model.

Tests whether within-group contact with the primary out-group predicts
stronger superordinate (Estonian) identification.

Direction handling:
  - Contact (Q51 / Q52, K4X1 / K4X2): inverted via (6 − raw) → higher = more contact
  - SI composite items: Q67_2, Q67_4, Q67_5 (2023) / K6X5_2, K6X5_3, K6X5_4 (2020).
    Q67_4 / K6X5_3 ("feel like a second-class citizen") reverse-coded via (5 − raw)
    before averaging. Resulting raw composite: lower = stronger belonging.
    For analysis we invert the composite via (5 − raw) so HIGHER = stronger
    belonging — matching the chart-label convention used by the dumbbell, mind
    map, and other user-facing visualizations.
  - Contact theory predicts POSITIVE slope: more contact → stronger belonging
    (same sign convention as the MIS regression in scripts 65/66).

Outputs:
  Console: coefficient, SE, 95% CI, t, p, R², N for each regression
  reports/Contact_Theory_Regression_SI.docx
  viz/fig_contact_theory_regression_si.jpg — 2×2 scatter+fit panels
"""
...
def si_composite_inverted(df, items, reverse_items, scale_max=4):
    """SI composite under user-facing convention (higher = more belonging).

    Step 1: reverse-code the item(s) in `reverse_items` via (scale_max + 1) − raw.
    Step 2: average pairwise across all items → raw SI composite (lower = more belonging).
    Step 3: invert the composite via (scale_max + 1) − raw so higher = more belonging.
    """
...
# ---------- Item codes ------------------------------------------------------
SI_2023      = ["Q67_2",  "Q67_4",  "Q67_5"]
SI_2023_REV  = ["Q67_4"]
SI_2020      = ["K6X5_2", "K6X5_3", "K6X5_4"]
SI_2020_REV  = ["K6X5_3"]
...
def build_pair(df, eth_val, contact_items, year):
    ...
    if year == 2020:
        si = si_composite_inverted(sub, SI_2020, SI_2020_REV)
    else:
        si = si_composite_inverted(sub, SI_2023, SI_2023_REV)
    contact = contact_composite(sub, contact_items)
    return pd.DataFrame({"SI": si, "Contact": contact}).dropna()
...
# ---------- Run regressions -------------------------------------------------
print("=" * 92)
print("CONTACT-THEORY REGRESSION CHECK — Superordinate Identity")
print("Model: SI (inverted: higher = stronger belonging) ~ Contact (higher = more contact)")
print("Contact theory predicts POSITIVE slope: more contact → stronger belonging")
print("=" * 92)
...
H("Contact-Theory Regression Check — Superordinate Identity",
  size=14, color=RGBColor(0x1f, 0x29, 0x37))

body(
    "Targeted bivariate regression of Superordinate Identity (sense of belonging to Estonian "
    "society) on frequency of contact with the primary out-group, run separately for each "
    "group × year cell. Companion to the SD: Primary Out-group, Minority Inclusion Support, "
    "and Belief in Inevitable Conflict regressions. Tests whether more contact predicts "
    "stronger superordinate belonging.",
    italic=True
)
doc.add_paragraph()
...
doc.add_paragraph()
body(
    "Note. Each row reports an OLS regression of the Superordinate Identity composite "
    "(Q67_4 / K6X5_3 reverse-coded via 5 − raw within the composite, then composite inverted "
    "via 5 − raw so higher = stronger belonging — matching user-facing chart conventions) on "
    "the Contact composite (inverted via 6 − raw, so higher = more frequent contact). For "
    "Russian respondents, contact is with Estonian-speakers (Q51 / K4X1); for Estonian "
    "respondents, with Russian-speakers (Q52 / K4X2). Contact theory predicts a POSITIVE "
    "slope (more contact → stronger belonging). Listwise deletion. Cross-sectional design; "
    "associations are not causal. Significance: *** p < .001, ** p < .01, * p < .05, "
    "ns = not significant.",
    italic=True, size=8
)

out_doc = ROOT / "reports" / "Contact_Theory_Regression_SI.docx"
doc.save(out_doc)
...
for r, df_pair, color, ax in panel_config:
    ...
    ax.scatter(df_pair["Contact"] + jx, df_pair["SI"] + jy,
...
axes[1, 1].set_xlabel("Contact composite (inverted: higher = more frequent contact)",
                       fontsize=9, color="#444")
axes[0, 0].set_ylabel("Superordinate Identity (inverted: higher = stronger belonging)",
                       fontsize=9, color="#444")
axes[1, 0].set_ylabel("Superordinate Identity (inverted: higher = stronger belonging)",
                       fontsize=9, color="#444")

fig.text(0.04, 0.965,
         "Contact-Theory Regression Check — Superordinate Identity ~ Contact",
...
fig.text(0.04, 0.940,
         ...
         fontsize=9, color="#444")
fig.text(0.04, 0.922,
         "Contact theory predicts a POSITIVE slope: more contact → stronger superordinate belonging.",
         fontsize=9, color="#444")
...
out_jpg = ROOT / "viz" / "fig_contact_theory_regression_si.jpg"
