"""
Estonian Respondents — Superordinate Identity Items Distribution Shapes
=========================================================================
Companion to code/32_si_density_russians.py — same three-panel KDE +
histogram layout, identical visual grammar, but restricted to Estonian
respondents.
...
Estonian respondents are largely at the high end of all three items
(in-group identity ceiling effects), so the distributional comparison
mostly tests whether the ceiling has tightened or loosened across waves.

Output: viz/fig_si_density_estonian.jpg  (300 DPI)
"""
...
# ---------- Load and filter (Estonian respondents only) ------------------
df23 = pd.read_csv(ROOT / "data" / "EIM23.csv")
df23 = df23[df23["ethnicity_binary"] == 0].copy()    # Estonian only
...
df20 = df20[df20["ethnicity_binary"] == 0].copy()
...
ITEMS = [
    # See script 32 for explanation: Q67_4 / K6X5_3 is the negatively-worded item;
    # raw direction already aligns with "higher = more belonging" so it is NOT
    # inverted here. Q67_2 and Q67_5 are positively-worded and need 5-raw.
    {
...
print("="*92)
print("ESTONIAN RESPONDENTS — Superordinate Identity items, distribution diagnostics")
...
for it in ITEMS:
    ...
    for tag, s in [("2020", it["s2020"]), ("2023", it["s2023"])]:
        d = diag(s)
        print(f"    Estonian {tag}: N={d['N']:4d}  M={d['M']:.3f}  SD={d['SD']:.3f}  "
...
def kde_overlay(ax, s2020, s2023, label2020, label2023):
    ...
    ax.bar(centers, h2023, width=BAR_W, align="center",
           color=EST, alpha=0.50, edgecolor="none", zorder=2)
    if len(s2020) > 5:
...
fig.text(0.04, 0.970,
         "Estonian Respondents — Superordinate Identity Items, Distribution Shapes 2020 → 2023",
...
out = ROOT / "viz" / "fig_si_density_estonian.jpg"
