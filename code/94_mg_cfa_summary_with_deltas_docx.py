...
Includes both between-group (Estonian vs Russian within each year) and
within-group (2020 vs 2023 within each ethnicity) MG-CFAs.

Δ values are computed as level-vs-previous-level differences:
  metric  − configural
  scalar  − metric
Δχ² and Δp use Satorra-Bentler scaled difference test (lavTestLRT).
ΔCFI and ΔRMSEA are simple differences.
...
Output: reports/MG_CFA_Fit_and_PCA_Summary_with_Deltas.docx
"""
...
OUT = ROOT / "reports" / "MG_CFA_Fit_and_PCA_Summary_with_Deltas.docx"
...
# Title
doc.add_heading("Multi-Group CFA: Fit Indices, Invariance Δ, and PC1 Variance", level=0)
...
p.add_run(
    ...
    "level-vs-previous-level differences. Δ values populated for metric "
    "(vs. configural) and scalar (vs. metric)."
).font.size = Pt(10)
...
p.add_run(
    ...
    "2020 (3 items) and 2023 (6 items); between-group MG-CFA fit per year, within-group "
    "MG-CFA not defined."
).font.size = Pt(10)


def render_fit_table(df_in):
    rows = []
...
r = foot.add_run(
    ...
    "Generated by code/94_mg_cfa_summary_with_deltas_docx.py."
)
r.italic = True
r.font.size = Pt(8)
