"""
Contact-Theory Regression Coefficient Plot — Minority Inclusion Support
========================================================================
Companion visualization to script 65 (which produced the 2×2 scatter+fit
panels). Same four regressions; here the slopes are aligned on a common
axis with 95% CI brackets, making cross-cell comparison direct.

Mirrors script 64's coefficient plot for the SD: Primary regression.

Direction reminder:
  - MIS inverted via (5 − raw); higher = more support for inclusion.
  - Contact inverted via (6 − raw); higher = more frequent contact.
  - Contact theory predicts POSITIVE slope (more contact → more support).

Output: viz/fig_contact_theory_coefplot_mis.jpg  (300 DPI)
"""
...
def build_pair(df, eth_val, contact_items, year):
    ...
    mis_inv     = composite_pairwise(sub, mis_items, invert_max=4)
    contact_inv = composite_pairwise(sub, contact_items, invert_max=5)
...
for label, df_pair, group in CELLS:
    ...
    model = sm.OLS(df_pair["MIS"], X).fit()
...
# Within-group dashed connectors (2020 → 2023)
...
# X-axis — keep range comparable in shape but smaller magnitudes than the SD chart
ax.set_xlim(-0.18, 0.40)
ax.set_xticks(np.arange(-0.15, 0.40, 0.05))
ax.set_xlabel("Slope b₁ — change in Minority Inclusion Support (inverted) per +1 unit of contact frequency",
...
for x in np.arange(-0.15, 0.40, 0.05):
    ax.axvline(x, color="#eee", linewidth=0.5, zorder=0)

# Title
fig.text(0.04, 0.965,
         "Contact-Theory Regression — Slopes with 95% Confidence Intervals (Minority Inclusion Support)",
         fontsize=13, fontweight="bold", ha="left")
fig.text(0.04, 0.940,
         "Bivariate OLS regression of MIS (inverted: higher = more support) on Contact (inverted: higher = more contact), within each group × year cell.",
         fontsize=8.5, color="#444", ha="left")
fig.text(0.04, 0.918,
         "Positive slope = contact-theory direction (more contact predicts more support for minority inclusion).",
         fontsize=8.5, color="#444", ha="left")
...
zero = mlines.Line2D([], [], color="#666", linewidth=1.0,
                     label="0 = no contact–support association")
...
fig.text(0.04, 0.010,
         "For Russian respondents, the MIS items refer to involvement of \"other nationalities\" (a category that includes the in-group); not a direct contact-theory test for that group. Significance: *** p<.001, ** p<.01, * p<.05, ns = not significant.",
         fontsize=7.0, color="#555", ha="left")

plt.subplots_adjust(left=0.13, right=0.985, top=0.86, bottom=0.16)

out_path = ROOT / "viz" / "fig_contact_theory_coefplot_mis.jpg"
