"""
Contact-Theory Regression Coefficient Plot — Superordinate Identity
======================================================================
Coefficient (forest) plot variant of script 69. Same four regressions; slopes
aligned on a common axis with 95% CI brackets.

Direction reminder:
  - SI inverted via (5 − raw); higher = stronger belonging.
  - Contact inverted via (6 − raw); higher = more contact.
  - Contact theory predicts POSITIVE slope.

Output: viz/fig_contact_theory_coefplot_si.jpg  (300 DPI)
"""
...
ax.set_xticks(np.arange(-0.15, 0.65, 0.05))
ax.set_xlabel("Slope b₁ — change in Superordinate Identity (inverted) per +1 unit of contact frequency",
...
for x in np.arange(-0.15, 0.55, 0.05):
    ax.axvline(x, color="#eee", linewidth=0.5, zorder=0)

# Title
fig.text(0.04, 0.965,
         "Contact-Theory Regression — Slopes with 95% Confidence Intervals (Superordinate Identity)",
         fontsize=13, fontweight="bold", ha="left")
fig.text(0.04, 0.940,
         "Bivariate OLS regression of SI (inverted: higher = stronger belonging) on Contact (inverted: higher = more contact), within each group × year cell.",
         fontsize=8.5, color="#444", ha="left")
fig.text(0.04, 0.918,
         "Positive slope = contact-theory direction (more contact predicts stronger superordinate belonging).",
         fontsize=8.5, color="#444", ha="left")
...
zero = mlines.Line2D([], [], color="#666", linewidth=1.0,
                     label="0 = no contact–belonging association")
...
out_path = ROOT / "viz" / "fig_contact_theory_coefplot_si.jpg"
