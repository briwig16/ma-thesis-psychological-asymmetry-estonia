"""
Contact-Theory Regression Coefficient Plot — Q68_3 "Understand opinions"
==========================================================================
Coefficient (forest) plot variant of script 71. Same four single-item
regressions; slopes aligned on a common axis with 95% CI brackets.

Direction reminder:
  - Q68_3 / K6X6_3 inverted via (5 − raw); higher = stronger agreement.
  - Contact inverted via (6 − raw); higher = more contact.
  - Contact theory predicts POSITIVE slope.

Output: viz/fig_contact_theory_coefplot_q68_3.jpg  (300 DPI)
"""
...
def build_pair(df, eth_val, contact_items, year):
    ...
    item_inv = 5 - to_num(sub[item_var])
...
for label, df_pair, group in CELLS:
    ...
    model = sm.OLS(df_pair["Item"], X).fit()
...
# Range chosen to fit all CIs + annotation text
ax.set_xlim(-0.20, 0.50)
ax.set_xticks(np.arange(-0.20, 0.55, 0.05))
ax.set_xlabel("Slope b₁ — change in Q68_3 / K6X6_3 \"Understand opinions\" per +1 unit of contact frequency",
...
for x in np.arange(-0.20, 0.55, 0.05):
    ax.axvline(x, color="#eee", linewidth=0.5, zorder=0)

# Title
fig.text(0.04, 0.965,
         "Contact-Theory Regression — Slopes with 95% CIs (Q68_3 \"Understand opinions\")",
         fontsize=13, fontweight="bold", ha="left")
fig.text(0.04, 0.940,
         "Bivariate OLS regression of Q68_3 / K6X6_3 (inverted: higher = stronger agreement) on Contact (inverted: higher = more contact), within each group × year cell.",
         fontsize=8.5, color="#444", ha="left")
fig.text(0.04, 0.918,
         "Positive slope = contact-theory direction (more contact predicts more support for understanding other nationalities' opinions).",
         fontsize=8.5, color="#444", ha="left")
...
# Footer
fig.text(0.04, 0.025,
         "Item-level analysis avoids the measurement-invariance complications of the composite-level MIS regression.",
         fontsize=7.0, color="#555", ha="left")
fig.text(0.04, 0.010,
         "Dashed within-group lines connect 2020 → 2023. Cross-sectional associations; no causal interpretation. Significance: *** p<.001, ** p<.01, * p<.05, ns = not significant.",
         fontsize=7.0, color="#555", ha="left")

plt.subplots_adjust(left=0.13, right=0.985, top=0.86, bottom=0.16)

out_path = ROOT / "viz" / "fig_contact_theory_coefplot_q68_3.jpg"
