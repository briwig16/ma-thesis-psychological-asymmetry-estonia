"""
Conditional (total) effect of contact at three language levels — computed
from the interaction MLR (Table 2 of MLR_Contact_Language_APA.docx).

For each cell, the interaction model fits:
    outcome = B0 + B1 × Contact_c + B2 × Language_c + B3 × (Contact_c × Language_c) + ε

The conditional effect of contact at a given language level L is:
    B_contact_total(L) = B1 + B3 × L

Evaluated at:
    Low language  (L = −1 SD of language)
    Mean language (L = 0)         ← equals B1
    High language (L = +1 SD)

Standard errors of the linear combinations are computed from the parameter
covariance matrix (HC3 robust). Significance and CIs follow.

Outputs:
  code/_conditional_contact_slopes.tsv
  reports/Conditional_Contact_Slopes_APA.docx
  viz/fig_conditional_contact_slopes.jpg
"""

import math
from pathlib import Path

import numpy as np
import pandas as pd
import pyreadstat
import statsmodels.api as sm
import matplotlib.pyplot as plt
import matplotlib.lines as mlines
from docx import Document
from docx.shared import Pt, Cm
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.enum.table import WD_ALIGN_VERTICAL
from docx.oxml import OxmlElement
from docx.oxml.ns import qn

ROOT = Path(__file__).parent.parent

df23 = pd.read_csv(ROOT / "data" / "EIM23.csv")
df23 = df23[df23["ethnicity_binary"].isin([0, 1])].copy().reset_index(drop=True)
df20, _ = pyreadstat.read_sav(
    str(ROOT / "data" / "EIM 2020_20.10.25.sav copy"), encoding="latin1"
)
df20["ethnicity_binary"] = df20.apply(
    lambda r: 0 if r.get("T9_1") == 1 else (1 if r.get("T9_2") == 1 else None),
    axis=1,
)
df20 = df20[df20["ethnicity_binary"].isin([0, 1])].copy().reset_index(drop=True)


def clean(df, items, reverse_items=None, scale_max=None, dk_code=9):
    sub = df[items].apply(pd.to_numeric, errors="coerce")
    sub = sub.where(sub != dk_code)
    if reverse_items:
        for it in reverse_items:
            sub[it] = (scale_max + 1) - sub[it]
    return sub.mean(axis=1, skipna=True)


def single_item(df, var, dk_code=9):
    s = pd.to_numeric(df[var], errors="coerce")
    return s.where(s != dk_code).astype(float)


def to_num(s, dk=9):
    s = pd.to_numeric(s, errors="coerce")
    return s.where(s != dk).astype(float)


CONTACT_2023 = {0: [f"Q52_{i}" for i in range(1, 7)], 1: [f"Q51_{i}" for i in range(1, 7)]}
CONTACT_2020 = {0: [f"K4X2_{i}" for i in range(1, 7)], 1: [f"K4X1_{i}" for i in range(1, 7)]}
LANG_2023 = {0: "Q71_2", 1: "Q71_1"}
LANG_2020 = {0: "K5_2",  1: "K5_1"}


def build_cell(spec, year, grp_code):
    df = df23 if year == 2023 else df20
    sub_df = df[df["ethnicity_binary"] == grp_code]
    if spec.get("single_item"):
        item = spec[f"items_{year}"]
        dk = spec.get(f"dk_{year}", 9)
        outcome = single_item(sub_df, item, dk_code=dk)
    elif isinstance(spec[f"items_{year}"], dict):
        grp_key = "E" if grp_code == 0 else "R"
        items = spec[f"items_{year}"][grp_key]
        rev = (spec[f"rev_{year}"][grp_key] if spec.get(f"rev_{year}") else None)
        outcome = clean(sub_df, items, rev, spec["smax"])
    else:
        items = spec[f"items_{year}"]
        rev = spec.get(f"rev_{year}")
        outcome = clean(sub_df, items, rev, spec["smax"])
    if spec["inv"]:
        outcome = (spec["smax"] + 1) - outcome
    contact_items = CONTACT_2023[grp_code] if year == 2023 else CONTACT_2020[grp_code]
    contact = 6 - clean(sub_df, contact_items)
    lang_col = LANG_2023[grp_code] if year == 2023 else LANG_2020[grp_code]
    lang = 7 - to_num(sub_df[lang_col])
    return pd.DataFrame({"value": outcome.values, "contact": contact.values,
                          "language": lang.values}).dropna()


SPECS = [
    {"name": "Superordinate Identity", "smax": 4, "inv": True,
     "items_2023": ["Q67_2","Q67_4","Q67_5"], "rev_2023": ["Q67_4"],
     "items_2020": ["K6X5_2","K6X5_3","K6X5_4"], "rev_2020": ["K6X5_3"]},
    {"name": "SD: Primary Out-group", "smax": 5, "inv": False,
     "items_2023": {"E": ["Q57_1","Q58_1","Q59_1"], "R": ["Q57_2","Q58_2","Q59_2"]},
     "items_2020": {"E": ["K4X7_1","K4X8_1","K4X9_1"], "R": ["K4X7_2","K4X8_2","K4X9_2"]}},
    {"name": "SD: General Out-group", "smax": 5, "inv": False,
     "items_2023": ["Q57_4","Q57_5","Q58_4","Q58_5","Q59_4","Q59_5"],
     "items_2020": ["K4X7_3","K4X8_3","K4X9_3"]},
    {"name": "Comparative Opportunity Assessment", "smax": 5, "inv": True,
     "items_2023": [f"Q44_{i}" for i in range(1,13)],
     "items_2020": [f"K3X1_{i}" for i in range(1,13)]},
    {"name": "Belief in Inevitable Conflict", "smax": 4, "inv": False,
     "items_2023": ["Q63_1","Q63_2","Q63_3","Q63_4"], "rev_2023": ["Q63_1","Q63_2"],
     "items_2020": ["K6X1_1","K6X1_2","K6X1_3","K6X1_4"], "rev_2020": ["K6X1_1","K6X1_2"]},
    {"name": "Minority Inclusion Support", "smax": 4, "inv": True,
     "items_2023": ["Q68_1","Q68_2","Q68_3"],
     "items_2020": ["K6X6_1","K6X6_2","K6X6_3"]},
    {"name": "Group ID Patterns", "smax": 5, "inv": False, "single_item": True,
     "items_2023": "Q66", "items_2020": "K6X4",
     "dk_2023": 9, "dk_2020": 6},
    {"name": "Territorial Attachment", "smax": 4, "inv": True, "single_item": True,
     "items_2023": "Q67_1", "items_2020": "K6X5_1"},
]


def stars(p):
    if pd.isna(p): return ""
    if p < .001: return "***"
    if p < .01:  return "**"
    if p < .05:  return "*"
    if p < .10:  return "⁺"
    return ""


def fmt_p(p):
    if pd.isna(p): return "—"
    if p < .001: return "< .001"
    return f"{p:.3f}".lstrip("0")


# ---------- Fit interaction model per cell + compute conditional slopes ---
rows = []
for spec in SPECS:
    for grp_code, grp_name in [(0, "Estonian"), (1, "Russian")]:
        for year in (2020, 2023):
            data = build_cell(spec, year, grp_code)
            if len(data) < 10: continue
            data["contact_c"]  = data["contact"]  - data["contact"].mean()
            data["language_c"] = data["language"] - data["language"].mean()
            data["cxl"] = data["contact_c"] * data["language_c"]
            sd_lang = data["language_c"].std(ddof=1)

            X = sm.add_constant(data[["contact_c", "language_c", "cxl"]])
            m = sm.OLS(data["value"], X).fit(cov_type="HC3")
            B1 = m.params["contact_c"]; B3 = m.params["cxl"]

            # Conditional slope at language level L:
            #   slope = B1 + B3 × L
            # Var(slope) = Var(B1) + L² × Var(B3) + 2L × Cov(B1, B3)
            cov = m.cov_params()
            var_B1 = cov.loc["contact_c", "contact_c"]
            var_B3 = cov.loc["cxl", "cxl"]
            cov_13 = cov.loc["contact_c", "cxl"]

            results_per_level = {}
            for label, L in [("low", -sd_lang), ("mean", 0.0), ("high", +sd_lang)]:
                slope = B1 + B3 * L
                var_slope = var_B1 + (L ** 2) * var_B3 + 2 * L * cov_13
                se_slope = math.sqrt(var_slope) if var_slope > 0 else float("nan")
                # Wald z-test
                z = slope / se_slope if se_slope > 0 else float("nan")
                # Two-sided p from normal approximation (large N)
                from scipy.stats import norm
                p = 2 * (1 - norm.cdf(abs(z))) if not math.isnan(z) else float("nan")
                ci_low  = slope - 1.96 * se_slope
                ci_high = slope + 1.96 * se_slope
                results_per_level[label] = {
                    "slope": slope, "SE": se_slope, "p": p,
                    "ci_low": ci_low, "ci_high": ci_high,
                }

            rows.append({
                "variable": spec["name"], "group": grp_name, "year": year,
                "N": int(m.nobs),
                "B1_main": B1, "B3_interaction": B3,
                "p_B3": m.pvalues["cxl"], "SD_language": sd_lang,
                # Low language
                "slope_low":     results_per_level["low"]["slope"],
                "SE_low":        results_per_level["low"]["SE"],
                "p_low":         results_per_level["low"]["p"],
                "CI_low_low":    results_per_level["low"]["ci_low"],
                "CI_high_low":   results_per_level["low"]["ci_high"],
                # Mean language
                "slope_mean":    results_per_level["mean"]["slope"],
                "SE_mean":       results_per_level["mean"]["SE"],
                "p_mean":        results_per_level["mean"]["p"],
                "CI_low_mean":   results_per_level["mean"]["ci_low"],
                "CI_high_mean":  results_per_level["mean"]["ci_high"],
                # High language
                "slope_high":    results_per_level["high"]["slope"],
                "SE_high":       results_per_level["high"]["SE"],
                "p_high":        results_per_level["high"]["p"],
                "CI_low_high":   results_per_level["high"]["ci_low"],
                "CI_high_high":  results_per_level["high"]["ci_high"],
            })

df_out = pd.DataFrame(rows)
df_out.to_csv(ROOT / "code" / "_conditional_contact_slopes.tsv",
               sep="\t", index=False, float_format="%.4f")
print(f"Saved TSV: {ROOT / 'code' / '_conditional_contact_slopes.tsv'}\n")


# ============================================================================
#                          APA WORD DOC
# ============================================================================
def _set_cell_border(cell, **kwargs):
    tc = cell._tc; tcPr = tc.get_or_add_tcPr()
    tcBorders = tcPr.find(qn("w:tcBorders"))
    if tcBorders is None:
        tcBorders = OxmlElement("w:tcBorders"); tcPr.append(tcBorders)
    for side in ("top","left","bottom","right"):
        spec_ = kwargs.get(side)
        elem = tcBorders.find(qn(f"w:{side}"))
        if elem is None:
            elem = OxmlElement(f"w:{side}"); tcBorders.append(elem)
        if spec_ is None:
            elem.set(qn("w:val"), "nil")
        else:
            elem.set(qn("w:val"), spec_.get("val","single"))
            elem.set(qn("w:sz"),  str(spec_.get("sz", 8)))
            elem.set(qn("w:color"), spec_.get("color","auto"))


def apply_apa_borders(table, header_rows=1):
    n = len(table.rows)
    for r_idx, row in enumerate(table.rows):
        for cell in row.cells:
            top = bottom = None
            if r_idx == 0:
                top = {"sz": 12, "val":"single", "color":"000000"}
            if r_idx == header_rows - 1:
                bottom = {"sz": 6, "val":"single", "color":"000000"}
            if r_idx == n - 1:
                bottom = {"sz": 12, "val":"single", "color":"000000"}
            _set_cell_border(cell, top=top, bottom=bottom, left=None, right=None)


def remove_table_style(table):
    tblPr = table._tbl.find(qn("w:tblPr"))
    if tblPr is not None:
        tblStyle = tblPr.find(qn("w:tblStyle"))
        if tblStyle is not None: tblPr.remove(tblStyle)
    for row in table.rows:
        for cell in row.cells:
            tcPr = cell._tc.get_or_add_tcPr()
            shd = tcPr.find(qn("w:shd"))
            if shd is not None: tcPr.remove(shd)


doc = Document()
sec = doc.sections[0]
sec.left_margin = sec.right_margin = Cm(1.2)
sec.top_margin = sec.bottom_margin = Cm(1.8)
sec.orientation = 1
sec.page_width  = Cm(29.7); sec.page_height = Cm(21.0)
doc.styles["Normal"].font.name = "Times New Roman"
doc.styles["Normal"].font.size = Pt(11)


def set_cell(cell, text, *, bold=False, italic=False, size=10,
              align=WD_ALIGN_PARAGRAPH.LEFT):
    cell.text = ""
    para = cell.paragraphs[0]; para.alignment = align
    lines = str(text).split("\n")
    for i, line in enumerate(lines):
        r = para.add_run(line)
        r.font.size = Pt(size); r.font.name = "Times New Roman"
        r.bold = bold; r.italic = italic
        if i < len(lines) - 1: r.add_break()


def set_cell_italics(cell, segments, *, size=10, align=WD_ALIGN_PARAGRAPH.LEFT):
    cell.text = ""
    para = cell.paragraphs[0]; para.alignment = align
    for text, bold, italic in segments:
        r = para.add_run(text)
        r.font.size = Pt(size); r.font.name = "Times New Roman"
        r.bold = bold; r.italic = italic


def merge_v(table, rows, col):
    cells = [table.rows[r].cells[col] for r in rows]
    merged = cells[0]
    for c in cells[1:]: merged = merged.merge(c)
    return merged


def H_table_number(text):
    p = doc.add_paragraph()
    r = p.add_run(text); r.bold = True; r.font.size = Pt(11); r.font.name = "Times New Roman"


def H_table_title(text):
    p = doc.add_paragraph()
    r = p.add_run(text); r.italic = True; r.font.size = Pt(11); r.font.name = "Times New Roman"


def table_note(segments, size=9):
    p = doc.add_paragraph()
    for text, bold, italic in segments:
        r = p.add_run(text)
        r.font.size = Pt(size); r.font.name = "Times New Roman"
        r.bold = bold; r.italic = italic


p = doc.add_paragraph()
r = p.add_run("Conditional (Total) Effect of Out-group Contact at Three Language Levels")
r.bold = True; r.font.size = Pt(15); r.font.name = "Times New Roman"

p = doc.add_paragraph()
r = p.add_run(
    "Conditional contact slopes computed from the interaction MLR "
    "(Table 2 of MLR_Contact_Language_APA.docx). The total effect of contact "
    "on the outcome at any language level L is B_contact,total(L) = B₁ + B₃ × L. "
    "Evaluated at low language (L = −1 SD), mean language (L = 0; equals B₁), "
    "and high language (L = +1 SD). Standard errors of these linear "
    "combinations are computed from the model's HC3 robust covariance matrix."
)
r.italic = True; r.font.size = Pt(10); r.font.name = "Times New Roman"
doc.add_paragraph()


# Table 1
H_table_number("Table 1")
H_table_title(
    "Conditional contact slopes at three language levels, per outcome × "
    "group × wave (Contact slope = B₁ + B₃ × Language)."
)

header = ["Outcome", "Group", "Wave", "n",
          "Contact slope at\nLow Lang. (−1 SD)",
          "Contact slope at\nMean Lang.",
          "Contact slope at\nHigh Lang. (+1 SD)",
          "B₃ (interaction)", "p (B₃)"]
tbl = doc.add_table(rows=1, cols=len(header))
for i, h in enumerate(header):
    if h == "n":
        set_cell_italics(tbl.rows[0].cells[i], [("n", True, True)], size=9,
                          align=WD_ALIGN_PARAGRAPH.CENTER)
    elif "p" == h.split()[0]:
        set_cell_italics(tbl.rows[0].cells[i],
                          [("p", True, True), (" (B₃)", True, False)],
                          size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    else:
        set_cell(tbl.rows[0].cells[i], h, bold=True, size=9,
                  align=WD_ALIGN_PARAGRAPH.CENTER)

ordered = []
for spec in SPECS:
    for grp in ("Estonian", "Russian"):
        for year in (2020, 2023):
            m = [r for r in rows if r["variable"] == spec["name"]
                                       and r["group"] == grp and r["year"] == year]
            if m: ordered.append(m[0])

row_idx = 1
outcome_starts = {}
for r in ordered:
    table_row = tbl.add_row().cells
    set_cell(table_row[0], r["variable"], size=9)
    set_cell(table_row[1], r["group"], size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[2], str(r["year"]), size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[3], str(r["N"]), size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[4], f"{r['slope_low']:+.3f} ({r['SE_low']:.3f}){stars(r['p_low'])}",
              size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[5], f"{r['slope_mean']:+.3f} ({r['SE_mean']:.3f}){stars(r['p_mean'])}",
              size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[6], f"{r['slope_high']:+.3f} ({r['SE_high']:.3f}){stars(r['p_high'])}",
              size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[7], f"{r['B3_interaction']:+.4f}{stars(r['p_B3'])}",
              size=9, align=WD_ALIGN_PARAGRAPH.CENTER)
    set_cell(table_row[8], fmt_p(r["p_B3"]), size=9,
              align=WD_ALIGN_PARAGRAPH.CENTER)
    if r["variable"] not in outcome_starts:
        outcome_starts[r["variable"]] = row_idx
    row_idx += 1

for spec in SPECS:
    start = outcome_starts.get(spec["name"])
    if start is None: continue
    end = start + 3
    if end < len(tbl.rows):
        merge_v(tbl, list(range(start, end + 1)), 0)
        set_cell(tbl.rows[start].cells[0], spec["name"], size=9)

remove_table_style(tbl); apply_apa_borders(tbl)

table_note([
    ("Note. ", False, True),
    ("Each conditional slope = B", False, False),
    ("₁", False, False),
    (" + B", False, False),
    ("₃", False, False),
    (" × Language. Mean language slope equals B", False, False),
    ("₁", False, False),
    (" exactly (since centered language = 0 at the mean). ", False, False),
    ("Low language = −1 SD of within-cell language; high language = +1 SD. "
     "Standard errors of the linear combinations computed from the model's "
     "HC3 robust covariance matrix. Significance from Wald z-tests. ", False, False),
    ("B", False, True),
    ("₃ tests whether the contact slope changes across language levels. "
     "*** ", False, False),
    ("p", False, True), (" < .001, ** ", False, False),
    ("p", False, True), (" < .01, * ", False, False),
    ("p", False, True), (" < .05, ⁺ ", False, False),
    ("p", False, True), (" < .10.", False, False),
])

out_doc = ROOT / "reports" / "Conditional_Contact_Slopes_APA.docx"
doc.save(out_doc)
print(f"Saved Word doc: {out_doc}")


# ============================================================================
#                              JPEG VISUALIZATION
# ============================================================================
plt.rcParams.update({
    "font.family": "DejaVu Sans",
    "font.size": 8,
    "axes.edgecolor": "#333",
    "axes.linewidth": 0.5,
})

EST_COLOR = "#2563eb"
RUS_COLOR = "#d97706"

OUTCOMES = [s["name"] for s in SPECS]
PRETTY = {
    "Superordinate Identity":             "Superordinate Identity",
    "SD: Primary Out-group":              "SD: Primary Out-group",
    "SD: General Out-group":              "SD: General Out-group",
    "Comparative Opportunity Assessment": "Comparative Opportunity",
    "Belief in Inevitable Conflict":      "Belief in Inevitable Conflict",
    "Minority Inclusion Support":         "Minority Inclusion Support",
    "Group ID Patterns":                  "Group ID Patterns",
    "Territorial Attachment":             "Territorial Attachment",
}
CELLS = [("Estonian", 2020), ("Estonian", 2023), ("Russian", 2020), ("Russian", 2023)]

# Determine common y-axis range
all_lo = pd.concat([df_out["CI_low_low"], df_out["CI_low_mean"], df_out["CI_low_high"]]).min()
all_hi = pd.concat([df_out["CI_high_low"], df_out["CI_high_mean"], df_out["CI_high_high"]]).max()
y_margin = 0.05
y_lim = (all_lo - y_margin, all_hi + y_margin)

fig, axes = plt.subplots(8, 4, figsize=(15.0, 22.0), dpi=300,
                          sharex=True, sharey=True)

for i, outcome in enumerate(OUTCOMES):
    for j, (group, year) in enumerate(CELLS):
        ax = axes[i, j]
        r_match = df_out[(df_out["variable"] == outcome) &
                          (df_out["group"] == group) &
                          (df_out["year"] == year)]
        if r_match.empty:
            ax.set_visible(False); continue
        r = r_match.iloc[0]
        color = EST_COLOR if group == "Estonian" else RUS_COLOR

        # X positions for low / mean / high
        x_positions = [0, 1, 2]
        slopes = [r["slope_low"], r["slope_mean"], r["slope_high"]]
        ci_lo  = [r["CI_low_low"], r["CI_low_mean"], r["CI_low_high"]]
        ci_hi  = [r["CI_high_low"], r["CI_high_mean"], r["CI_high_high"]]
        p_vals = [r["p_low"], r["p_mean"], r["p_high"]]

        # CI error bars
        ax.errorbar(x_positions, slopes,
                    yerr=[[s - lo for s, lo in zip(slopes, ci_lo)],
                          [hi - s for s, hi in zip(slopes, ci_hi)]],
                    color=color, marker="o", markersize=7,
                    markerfacecolor=color, markeredgecolor="white",
                    markeredgewidth=0.8, linewidth=1.8,
                    capsize=3, capthick=0.8, zorder=3)

        # Significance stars above each point
        for x, s, p in zip(x_positions, slopes, p_vals):
            star = stars(p)
            if star:
                ax.text(x, s, star, ha="center", va="bottom",
                        fontsize=7, color="#222", fontweight="bold")

        # Reference at 0
        ax.axhline(0, color="#999", linewidth=0.5, linestyle="--", zorder=1)

        # B₃ annotation
        ax.text(0.04, 0.97,
                f"B₃ = {r['B3_interaction']:+.3f}\n{fmt_p(r['p_B3'])} {stars(r['p_B3'])}",
                transform=ax.transAxes, ha="left", va="top",
                fontsize=7, color="#222", family="monospace",
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white",
                          edgecolor="#aaa", linewidth=0.4, alpha=0.93))

        ax.set_xticks(x_positions)
        ax.set_xticklabels(["Low\nLang", "Mean\nLang", "High\nLang"], fontsize=7)
        ax.set_xlim(-0.5, 2.5)
        ax.set_ylim(y_lim)
        for s_ in ("top", "right"):
            ax.spines[s_].set_visible(False)
        ax.tick_params(axis="both", labelsize=7, color="#888")
        ax.grid(True, axis="y", linestyle=":", linewidth=0.3, color="#eee", zorder=0)

        if i == 0:
            ax.set_title(f"{group} {year}", fontsize=10, fontweight="bold",
                         pad=8, color="#333")

# Row labels
for i, outcome in enumerate(OUTCOMES):
    axes[i, 0].set_ylabel(PRETTY[outcome], fontsize=9, fontweight="bold",
                          color="#222", labelpad=8)

# Bottom x-axis label
for j in range(4):
    axes[7, j].set_xlabel("Language level", fontsize=8, color="#555")

# Global title
fig.text(0.04, 0.985,
         "Conditional Contact Slopes at Three Language Levels — by Outcome × Group × Year",
         fontsize=15, fontweight="bold", ha="left")
fig.text(0.04, 0.973,
         "Each panel shows the total effect of contact on the outcome, B_total(L) = B₁ + B₃ × L, evaluated at low (−1 SD), mean (0), and high (+1 SD) language levels.",
         fontsize=9, color="#444", ha="left")
fig.text(0.04, 0.963,
         "Error bars = 95% CI on the linear combination. Three dots forming a line ≈ no Contact × Language moderation; sloping pattern = language modifies contact's effect.",
         fontsize=9, color="#444", ha="left")

handles = [
    mlines.Line2D([], [], marker="o", color=EST_COLOR, markerfacecolor=EST_COLOR,
                  markeredgecolor="white", markersize=8, linewidth=1.8,
                  label="Estonian respondents"),
    mlines.Line2D([], [], marker="o", color=RUS_COLOR, markerfacecolor=RUS_COLOR,
                  markeredgecolor="white", markersize=8, linewidth=1.8,
                  label="Russian respondents"),
]
fig.legend(handles=handles, loc="upper right", bbox_to_anchor=(0.99, 0.97),
           frameon=False, fontsize=9, ncol=1, handlelength=2.0)

fig.text(0.04, 0.013,
         "Out-group contact and out-group language ability, both mean-centered within each cell. Standard errors from HC3 robust covariance matrix. Significance: *** p<.001, ** p<.01, * p<.05, ⁺ p<.10.",
         fontsize=7.5, color="#555", ha="left")

plt.subplots_adjust(left=0.13, right=0.985, top=0.95, bottom=0.025,
                    wspace=0.12, hspace=0.30)

out_jpg = ROOT / "viz" / "fig_conditional_contact_slopes.jpg"
plt.savefig(out_jpg, dpi=300, format="jpg", facecolor="white",
            pil_kwargs={"quality": 95})
print(f"Saved chart: {out_jpg}")
