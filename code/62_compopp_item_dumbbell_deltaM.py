...
==========================================================================
Variant of script 61 that uses ΔM (raw mean difference, M_Est − M_Rus) in
the right column instead of Cohen's d.

ΔM is appropriate here because every Q44 item shares the same 1–5 response
scale, so raw point differences are directly interpretable ("Estonians scored
0.78 scale-points higher than Russians"). Standardised effect sizes add value
when comparing across constructs with different scales; that's not needed for
within-battery item comparison.

Significance is still computed via Welch's t-test (same as the d-based chart).

Output: viz/fig_compopp_item_dumbbell_deltaM.jpg  (300 DPI)
"""
...
for i, label in enumerate(ITEM_LABELS, start=1):
    ...
    delta_m_20 = e20.mean() - r20.mean()
    delta_m_23 = e23.mean() - r23.mean()
    ...
    rows.append({
        ...
        "dM20": delta_m_20, "dM23": delta_m_23,
...
# ---------- Diagnostics -----------------------------------------------------
print("=" * 100)
print("Q44 / K3X1 — Comparative Opportunity Assessment item-level dumbbell data (ΔM variant)")
print("Scale: 1 = much better for Estonians, 3 = equal, 5 = much better for other nationalities")
print("ΔM = M(Estonian) − M(Russian) on the raw 1–5 scale; positive ΔM = Estonians scored higher")
print("=" * 100)
for r in rows:
    print(f"  Q44_{r['idx']:<2d}  {r['name']:<32s}  "
          f"Est20={r['e20_M']:.2f}  Rus20={r['r20_M']:.2f}  ΔM20={r['dM20']:+.2f} {r['s20']:<3s}    "
          f"Est23={r['e23_M']:.2f}  Rus23={r['r23_M']:.2f}  ΔM23={r['dM23']:+.2f} {r['s23']}")
...
fig.text(0.03, 0.015,
         "ΔM = M(Estonian) − M(Russian) on the raw 1–5 scale; positive ΔM means Estonians scored higher (perceived more equality). Significance via Welch's t-test: *** p<.001, ** p<.01, * p<.05, ns = not significant.",
         fontsize=7.0, color="#555", ha="left")

out_path = ROOT / "viz" / "fig_compopp_item_dumbbell_deltaM.jpg"
